...
Subject: AutoScientist update: SciState Graph, Medea integration, and SciFlow Policy model
...
I also integrated Medea into the runtime and connected public biomedical tools plus ToolUniverse/OpenTargets paths where available. I ran a 50-task benchmark harvest; all 50 runs completed, exercising Medea smoke integration, public biomedical tools, local board/replay logging, SciState memory capture, and SciFlow training traces. The exported SciState Graph now contains 1,106 nodes, 2,795 edges, 69 hypotheses, 615 entities, and 207 proposed experiments.
...
- Medea/tool smoke run: [medea_tool_smoke_run_2a5da95eefb4.json](../artifacts/2026-05-20-autoscientist-runtime/medea_tool_smoke_run_2a5da95eefb4.json)

The next steps I think matter most are: run a stronger benchmark suite with live frontier and open-source reasoning models, add expert or rubric-based biomedical scoring, compare against no-memory and no-policy ablations, expand ToolUniverse/OpenTargets and Medea beyond smoke tests, and train SciFlow Policy on higher-quality traces to test whether it improves real autonomous-science workflows.
